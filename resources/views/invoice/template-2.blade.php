...
<body class="bg-gray-50 text-gray-800 font-sans min-h-screen py-10 px-4">
    <div class="no-print max-w-4xl mx-auto flex justify-end mb-4 space-x-4">
        <button onclick="window.print()" class="px-4 py-2 bg-theme-primary text-white rounded hover:opacity-90 transition">{{ __('Print Invoice') }}</button>
        <a href="{{ url()->previous() }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 transition">{{ __('Back') }}</a>
    </div>
    ...
    <div class="max-w-4xl mx-auto bg-white p-10 shadow-lg rounded-lg border border-gray-200">
        ...
        <div class="mb-8">
            <img src="{{ asset('storage/settings/' . settings('invoice_header')) }}" alt="{{ __('Invoice Header') }}" class="w-full h-auto rounded-t-lg">
        </div>
        <div class="mb-8">
            <h1 class="text-4xl font-extrabold text-theme-primary uppercase tracking-wider">{{ $entity ?? __('INVOICE') }}</h1>
        </div>
        @else
        <div class="flex justify-between items-center border-b-4 border-theme-primary pb-8 mb-8">
            <div class="w-1/2">
                @if(settings('site_logo'))
                    <img src="{{ asset('images/' . settings('site_logo')) }}" alt="{{ __('Company Logo') }}" class="h-20 object-contain mb-4">
                @endif
                <h1 class="text-4xl font-extrabold text-theme-primary uppercase tracking-wider">{{ $entity ?? __('INVOICE') }}</h1>
            </div>
            <div class="w-1/2 text-right">
                ...
                @if(settings('company_tax'))
                    <p class="text-sm text-gray-500 mt-1">{{ __('Tax ID:') }} <span class="font-medium text-gray-700">{{ settings('company_tax') }}</span></p>
                @endif
            </div>
        </div>
        @endif
        ...
        <div class="flex justify-between bg-theme-secondary p-6 rounded-lg mb-8">
            <div>
                <p class="text-xs text-gray-500 uppercase tracking-widest font-semibold mb-1">{{ __('Invoice Number') }}</p>
                <p class="text-lg font-bold text-gray-900">{{ $data->reference ?? __('REF-XXXX') }}</p>
            </div>
            <div class="text-right">
                <p class="text-xs text-gray-500 uppercase tracking-widest font-semibold mb-1">{{ __('Date of Issue') }}</p>
        ...
        <div class="mb-10 px-2">
            <h3 class="text-xs text-gray-500 uppercase tracking-widest font-bold border-b border-gray-200 pb-2 mb-4">{{ __('Billed To') }}</h3>
            <p class="text-xl font-bold text-gray-800 mb-1">{{ $data->customer->name ?? $data->supplier->name ?? __('Client Name') }}</p>
            ...
            @if(isset($data->customer->tax_number))
                <p class="text-sm text-gray-600 mt-1">{{ __('Tax ID: :param_1', ['param_1' => $data->customer->tax_number]) }}</p>
            @endif
        </div>
        ...
        <div class="overflow-hidden rounded-lg border border-gray-200 mb-8">
            <table class="w-full text-left bg-white">
                <thead class="bg-theme-primary text-white">
                    <tr>
                        <th class="py-4 px-6 font-semibold text-sm tracking-wide">{{ __('Item Description') }}</th>
                        <th class="py-4 px-6 font-semibold text-sm tracking-wide text-center">{{ __('Qty') }}</th>
                        <th class="py-4 px-6 font-semibold text-sm tracking-wide text-right">{{ __('Price') }}</th>
                        @if(checkInvoiceControl('show_discount'))
                            <th class="py-4 px-6 font-semibold text-sm tracking-wide text-right">{{ __('Discount') }}</th>
                        @endif
                        @if(checkInvoiceControl('show_order_tax'))
                            <th class="py-4 px-6 font-semibold text-sm tracking-wide text-right">{{ __('Tax') }}</th>
                        @endif
                        <th class="py-4 px-6 font-semibold text-sm tracking-wide text-right">{{ __('Total') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @forelse($data->details ?? [] as $detail)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="py-4 px-6 text-sm font-medium text-gray-900">{{ $detail->product->name ?? __('Product Name') }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="py-8 px-6 text-center text-gray-400 italic">{{ __('No line items found for this invoice.') }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="flex justify-end mb-12">
            <div class="w-full md:w-2/5 bg-gray-50 p-6 rounded-lg border border-gray-100">
                <div class="space-y-3 mb-4">
                    @if(checkInvoiceControl('show_order_tax'))
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">{{ __('Order Tax') }}</span>
                ...
                <div class="space-y-2 border-t border-gray-200 pt-4">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">{{ __('Amount Paid') }}</span>
                        <span class="font-semibold text-green-600">{{ format_currency($data->paid_amount) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500 font-bold">{{ __('Balance Due') }}</span>
        ...
        <div class="mt-8">
            <img src="{{ asset('storage/settings/' . settings('invoice_footer')) }}" alt="{{ __('Invoice Footer') }}" class="w-full h-auto rounded-b-lg">
        </div>
        ...
        <div class="border-t border-gray-200 pt-8 mt-8 text-center">
            ...
            <p class="text-xs text-gray-400 mt-2">{{ __('Thank you for your business!') }}</p>
        </div>
        @endif
    </div>
</body>
