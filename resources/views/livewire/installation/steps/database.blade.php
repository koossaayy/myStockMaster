<div class="space-y-8">
    <div class="text-center">
        ...
        <h3 class="text-2xl font-bold text-orange-900 font-display">{{ __('Database Configuration') }}</h3>
        <p class="mt-2 text-orange-600">{{ __('Connect MyStockMaster to your database. MySQL is recommended for production.') }}</p>
    </div>
                    ...
                    <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div class="flex items-center">
                            ...
                            <p class="text-sm text-green-700">
                                <strong>{{ __('Desktop mode detected.') }}</strong> {{ __('SQLite will be used automatically. No database configuration needed.') }}
                            </p>
                        </div>
                    </div>
                @else
                    <div>
                        <label class="block text-sm font-medium text-orange-700 mb-3">{{ __('Database Type') }}</label>
                    ...
                    @if($database['connection'] !== 'sqlite')
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div>
                                <label for="db_host" class="block text-sm font-medium text-orange-700">{{ __('Host') }}</label>
                            ...
                            <div>
                                <label for="db_port" class="block text-sm font-medium text-orange-700">{{ __('Port') }}</label>
                            ...
                            <div>
                                <label for="db_username" class="block text-sm font-medium text-orange-700">{{ __('Username') }}</label>
                            ...
                            <div>
                                <label for="db_password" class="block text-sm font-medium text-orange-700">{{ __('Password') }}</label>
                    ...
                    <div>
                        <label for="db_name" class="block text-sm font-medium text-orange-700">{{ __('Database Name') }}</label>
            ...
            <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded-xl">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-green-700">
                            @if($isDesktopMode)
                                {{ __('SQLite is ready. Click') }} <strong>Next</strong> {{ __('to continue — database setup is automatic.') }}
                            @else
                                {{ session('connection_success') ?? __('Connection successful! Your database is ready. Click Next to continue.') }}
                            @endif
                        </p>
                    </div>
                </div>
