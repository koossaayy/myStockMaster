<div class="text-center">
    ...
    <h3 class="mt-6 text-3xl font-bold text-orange-900 font-display">{{ __('🎉 Installation Complete!') }}</h3>
    <p class="mt-3 text-lg text-orange-600 max-w-md mx-auto">
        {{ __('Congratulations! Your Stock Management system is now ready. You can now manage your inventory, add products, and start processing orders.') }}
    </p>
    
    <div class="mt-8 bg-green-50 border border-green-200 rounded-xl p-6 max-w-sm mx-auto">
        <h4 class="font-semibold text-green-800 mb-2">{{ __("What's next?") }}</h4>
        <ul class="text-sm text-green-700 space-y-1 text-left">
            <li>{{ __('Set up your product categories') }}</li>
            <li>{{ __('Add your inventory items') }}</li>
            <li>{{ __('Configure suppliers and customers') }}</li>
            <li>{{ __('Start managing your stock!') }}</li>
        </ul>
    </div>
