        ...
        <div x-show="step === 3" style="display: none;">
            ...
            <div class="flex flex-wrap mb-4">
                <div class="w-full px-2 my-2">
                    ...
                    <x-media-upload title="{{ __('Image') }}" name="form.image" wire:model="form.image" :file="$form->image" :preview="$form->image && is_string($form->image) ? asset('images/products/' . $form->image) : null" single types="PNG / JPEG / WEBP" fileTypes="image/*" />
                    <x-input-error :messages="$errors->get('form.image')" for="form.image" class="mt-2" />
                </div>
                <div class="w-full px-2 my-2">
                    <x-label for="gallery" :value="__('Product Gallery')" />
                    <x-media-upload title="{{ __('Gallery') }}" name="form.gallery" wire:model="form.gallery" :file="$form->gallery" :preview="is_array($form->gallery) ? collect($form->gallery)->filter(fn($i) => is_string($i))->map(fn($i) => asset('images/products/' . $i))->toArray() : []" multiple types="PNG / JPEG / WEBP" fileTypes="image/*" />
